...
We run the PNMF algorithm. $K$, the number of low dimension, is a key parameter that needs to be pre-specified by users. For a typical scRNA-seq data, $K=20$ is recommended in our *scPNMF* paper. But here we use $K=10$ for demonstration purpose in the vignette.
```{r PNMF}
...
```{r infogene}
ig <- getInfoGene(W_select, M = 50, by_basis = FALSE, return_trunW = TRUE, dim_use = NULL)
print(ig$InfoGene)
```
