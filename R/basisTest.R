...
basisTest <- function(S,
  ...
  if(return_fig) {
    ...
    p_scatter <- dat_long %>% ggplot(aes(y = Score, x = cell_lib_size)) + geom_point(alpha = 0.5, size = 0.2) + facet_wrap(~basis,  scales = "free", ncol = ncol) + xlab("Cell lib size") + ggtitle("Correlations with cell library sizes") +theme_bw() + theme(plot.title = element_text(face = "bold", size=12, hjust = 0.5), legend.position = "bottom", aspect.ratio = 1, axis.text.x = element_text(angle = 45, hjust = 1)) + guides(color = guide_legend(override.aes = list(size=5)))
    
    p_hist <- dat_long %>% ggplot(aes(x = Score)) + geom_histogram(color = "black", fill = "white") + facet_wrap(~basis, scales = "free", ncol = ncol) + ggtitle("Multimodality") +theme_bw() + theme(plot.title = element_text(face = "bold", size=12, hjust = 0.5), legend.position = "bottom", aspect.ratio = 1)+ guides(color = guide_legend(override.aes = list(size=5))) + ylab("Count")
    
    p <- gridExtra::grid.arrange(p_scatter, p_hist, nrow = 1)
